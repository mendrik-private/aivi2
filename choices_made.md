...
113. **`aivi run` ships first as a static GTK slice, not a fake reactive runtime:** The CLI now runs real AIVI markup only when the selected view is already fully static after lowering. Dynamic setters, event hooks, and control nodes such as `<show>`, `<each>`, `<match>`, and `<with>` are rejected explicitly because the current executor/runtime seam still expects an external driver for those inputs and would otherwise imply behavior the implementation does not yet have.
...
115. **Broader GTK run support remains blocked by real later-layer gaps, not CLI glue:** Repeated/template instances still share setter inputs in the current executor model, so per-instance dynamic property evaluation would be dishonest today, and backend runtime evaluation still lacks general user-defined closed-sum constructor execution for common markup/control expressions. Those are explicit follow-on implementation tasks rather than silent limitations of the new `run` command.
