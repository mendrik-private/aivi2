...
69. **Expected-result checking now includes ordinary application:** When the compiler already knows the expected result type and the callee has a known arrow type, it now checks ordinary function application bidirectionally instead of waiting for full argument inference. That lets the general typechecker and concrete source-option checking accept cases like `keep None` under a known `Option Int` expectation and report result mismatches earlier, while user-authored instances remain deferred because `aivi-syntax` still does not lex or parse an `instance` keyword even though HIR reserves the item shape.
