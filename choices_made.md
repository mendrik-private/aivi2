...
53. **Narrow provider contract declarations:** Because the RFC still lacks a full custom-provider declaration chapter, the compiler now takes the smallest coherent declaration surface: a top-level `provider qualified.name` item with an optional indented member body. Only `wakeup: timer | backoff | sourceEvent | providerTrigger` lowers today; built-in provider keys, unqualified names, unknown fields, and duplicate `wakeup` members diagnose immediately, and declarations stay separate from `@source` use-site resolution until the queued resolution slice lands.
