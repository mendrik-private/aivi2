...
- Inline-pipe `Case`, `TruthyFalsy`, and `Tap` stages now have Cranelift emission code. Pattern matching covers constructor, wildcard, integer, record, tuple, and list patterns. List pattern rest bindings use `aivi_list_slice`. The remaining gap is recursive self-referencing functions (no loop/tail-call support).
